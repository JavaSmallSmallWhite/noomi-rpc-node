...
import {Client as Zookeeper, createClient, Exception, Option, Stat, Event} from "node-zookeeper-client";

/**
 * zookeeper工具类
 */
export class ZookeeperUtils {
    ...
    public static createZookeeper(zookeeperConnectConfig?: ZookeeperConnectConfig): Zookeeper {
        ...
        if (!zookeeperConnectConfig) {
            ...
            try {
                ...
                const client: Zookeeper = createClient(connectString, options);
                client.connect();
     ...
     */
    public static async createNode(zookeeper: Zookeeper, node: ZookeeperNode, watcher: (event: Event) => void, createMode: number): Promise<boolean> {
        ...
        if (!isExist) {
            const result: Error | Exception | string = await new Promise((resolve, reject): void => {
                zookeeper.create(node.nodePath, node.data, createMode, function (error: Error | Exception, path: string): void {
                    if (error) {
                        reject(error);
                    }
                    resolve(path);
     ...
     */
    public static async exist(zookeeper: Zookeeper, nodePath: string, watcher: (event: Event) => void): Promise<boolean> {
     ...
     * @param watcher 是否监控
     */
    public static async getChildren(zookeeper: Zookeeper, serviceNode: string, watcher: (event: Event) => void): Promise<string[]>  {
        const result: string[] | Error | Exception = await new Promise((resolve, reject): void => {
            zookeeper.getChildren(serviceNode, watcher, function (error: Error | Exception, children: string[]): void {
                if (error) {
                    reject(error)
                }
                resolve(children);
