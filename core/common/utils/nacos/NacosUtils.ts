...
import {Host, Hosts, Instance, NacosNamingClient, NacosNamingClientConfig} from "nacos";
...
export class NacosUtils {
     ...
     */
    public static createNacosRegistryCenter(nacosRegistryConnectConfig?: NacosRegistryConnectConfig): NacosNamingClient {
     ...
     */
    public static async getAllInstances(nacos: NacosNamingClient, serviceName: string, groupName?: string, clusters?: string, subscribe?: boolean): Promise<Host[]> {
        try {
            if (!this.clientIsReady) {
                await nacos.ready();
            }
            const hosts: Host[] = await nacos.getAllInstances(serviceName, groupName, clusters, subscribe);
